(function($){
	...
	{
		nodes[i]={top:0,left:0,width:rects[i].width,height:rects[i].height,mostRight:true,next:null,previous:null};
	}
		...
		for(var i=0;i<rects.length;i++)
	{
		...
		if(!tryPlace($('.tagRect')[ran],nodes[ran],this))
		continue;
	flag=true;
	break;
	}
	...
	if(opts.rotate && $.fn.rotate)
		$('.tagRect').children().rotate({
			bind: 
		{ 
			mouseover : function() { 
					    $(this).rotate({animateTo:-$(this).parent().attr('rotate')})
				    },
			mouseout : function() { 
	...
	{
		var flag=false;
		if(right>parent.offsetWidth)
			return true;
		$(parent).children().each(function(){
			if(this==widget)
			return;
		if (!((left > (this.offsetLeft+this.offsetWidth)) || (right<this.offsetLeft)||
				(bottom < (this.offsetTop)) || (top > (this.offsetTop+this.offsetHeight))))
			flag=true;
		});
		return flag;
	...
	function insertPlaceNode(headPlaceNode,top,left,width,height)
	{
		...
		while(placeNode)
		{
			...
			{
				var newNode={top:top,left:left,width:width,height:height,mostRight:false,next:placeNode.next,previous:placeNode};
				if(placeNode.next)
					placeNode.next.previous=newNode;
	...
	function addRect(parent,startPoint,endPoint,height,definedSize)
	{
		...
		rects[rects.length]={top:top,left:left,width:edgeZ,height:30,angle:-50};
	}
